...
Use only Remote Commander on U24:

- device: u24-gpt
- target user: gpt
- repository/workdir: /home/gpt/projects/ah-there-it-is

All Git/edit/Python/Just/test operations must stay inside that exact repository.

Do not use direct-shell for this batch. Do not switch users. Do not use sudo or su.

Before mutation verify:

    test "$(id -un)" = "gpt"
    test "$HOME" = "/home/gpt"
    cd /home/gpt/projects/ah-there-it-is || exit 1
    test "$(git rev-parse --show-toplevel)" = "/home/gpt/projects/ah-there-it-is"

The repository must be clean and origin/main must equal the issued start-main before the implementation branch is created.

If the repository is dirty, checked out for another active task, unavailable to device u24-gpt, or its main does not match the issued start-main, stop and report the infrastructure conflict. Do not reuse another checkout and do not repair ownership or another session's state.

Do not activate the venv and do not export PATH.

Direct Python commands use .venv/bin/python.
Just recipes use plain just.
