...
Stages 0–13 are complete. The application regression pipeline covers the full 40-case corpus with independent persisted-state/event postconditions, agent turns are transactionally atomic, chat submissions are retry-safe through persisted idempotency keys plus explicit audited recovery, and the local SQLite store has validated WAL-safe backup/restore plus bidirectional versioned portable inventory/history export/import. `inventory-portable-v1` is now protected by a hand-authored compatibility fixture and explicit version dispatch. Provider/model compatibility remains a separate contract pipeline. Stage 14 will remove the remaining source-checkout/current-working-directory dependency from Alembic migration and portable-import execution by shipping and using packaged migration resources.

Voice, Telegram, images, QR, MCP, PWA, embeddings, and multi-user support remain deferred product features and are not part of Stage 14.
