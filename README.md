...
Normal CI jobs never receive provider secrets. Manual `workflow_dispatch` selects `groq` or `gemini`, a `smoke`, `tool-discipline`, or `representative` suite, and `prompt_variant=v1|v2-strict`; only the selected provider job runs and receives its own secret. Artifact names include the prompt variant so baseline and variant reports remain distinct. `smoke` runs `find-01` + `move-01`; `tool-discipline` runs `find-01` + `create-01` + `ambiguity-01`; `representative` additionally includes move/history coverage. Groq/Qwen live **benchmarking** uses temperature 0.2, top_p 0.95, `seed=424242`, `max_completion_tokens=256`, `reasoning_effort=none`, hidden reasoning, and provider tool validation disabled so malformed/unknown tool calls can be returned to the application's own authoritative dispatcher as structured errors instead of aborting the HTTP request. These settings are captured in run metadata and are an evaluation profile, not a blanket production recommendation. The smaller completion budget and instruct mode are deliberate: measured free-tier limits are 7000 input tokens/minute and 1000 output tokens/minute. OpenAI-compatible traces also retain a strict whitelist of provider rate-limit headers (limit/remaining/reset for requests and tokens), so quota pressure is visible before a 429; authentication and unrelated response headers are never persisted. Long provider `Retry-After` windows are never allowed to consume the whole evaluation job: the transport has an explicit retry-delay cap, and a hard provider quota is classified separately. `live_eval` records the failed case, aborts the remaining batch without more provider calls, and preserves the remaining case IDs in the partial report.
