...
This onboarding path is intentionally different from both recovery mechanisms below: bootstrap creates new current-domain state from declarative facts, portable import reconstructs an inventory/history archive with preserved stable IDs/timestamps, and SQLite backup/restore is the full-fidelity disaster-recovery path for every application table.
...
The current `inventory-portable-v2` JSON contains category/location trees, items, aliases, tags, structured attributes, explicit location status, current locations, and domain history events with stable IDs/timestamps. The frozen v1 format remains importable; import derives location status from its older Item facts. It intentionally excludes conversations, chat-request idempotency/recovery state, agent/evaluation/experiment traces, and provider metadata from reconstruction. SQLite backup remains the full-fidelity disaster-recovery snapshot for every application table.
...
The parser rejects unknown format identifiers, unknown structural fields, malformed section types, duplicate IDs, dangling parent/item/location references, self-parent links, hierarchy cycles, normalized duplicate sibling names, invalid states/timestamps, and other portable-domain integrity failures. `inventory-portable-v1` remains a frozen compatibility contract, with v2 handled through explicit format-version dispatch. A successful `export -> import -> export` is tested for semantic equality; `exported_at` is intentionally volatile and alias/tag list ordering is semantically irrelevant. Re-exporting an older v1 archive from a newly migrated database records the current database revision in `source.alembic_revision`; the older source revision remains import metadata and does not select migration code.


## Current scope

Stages 0–26 are complete. The browser can create, correct, and discover Items, Categories, and Locations without an LLM while the domain/service layer owns identity, tree, history, transaction, and FTS rules. Stage 26 presents Item condition/state separately from Location status, labels condition unknown separately from location unknown, and shows `known`, `unknown`, `in_use`, and `not_applicable` explicitly. Manual move, take, mark-unknown, discard, sold, and reactivation actions use InventoryService transitions; reactivation requires a non-terminal condition/state plus an explicit known Location or explicit unknown choice. The blank Items catalog defaults to active Items with lifecycle and location-unknown filters; nonblank search still returns terminal Items by default. Evidence-based location suggestions are labeled as inferences and shown only for location-unknown Items. The provider-independent 58-case scenario suite, explicit recovery/doctor tooling, CWD-independent installed runtime, and 1000-item / 200-location structural scale coverage remain in place.
