...
- GitHub Actions live testing uses the protected `live-llm-test` Environment and a manual provider choice. Gemini uses `GEMINI_API_KEY`, `GEMINI_MODEL`, and `GEMINI_BASE_URL`; Groq uses `GROQ_API_KEY`, `GROQ_MODEL`, and `GROQ_BASE_URL`. Ordinary CI jobs receive no provider secrets, and the selected live job receives only its provider's secret.
