...
- A deterministic Groq benchmark profile is now prepared for repeat prompt tests: `temperature=0.2`, `seed=424242`, `reasoning_effort=none`, `max_completion_tokens=256`, and `disable_tool_validation=true`. The latter lets hallucinated/unknown tool calls reach the application's authoritative `ToolDispatcher` as structured errors instead of making Groq abort the HTTP request. OpenAI-compatible response metadata now records the actual response model and `system_fingerprint`; `live_compare` reports backend-fingerprint changes between baseline and variant.
- Added a quota-aware `tool-discipline` suite (`find-01`, `create-01`, `ambiguity-01`) for cheap repeat testing of the exact failure family seen in the first v1/v2 comparison. Groq's live response exposed a 200,000 TPD ceiling for `qwen/qwen3.8-27b`; a deterministic v1 attempt began when about 199.6k tokens were already consumed and therefore produced only quota failures. Treat that run as quota evidence, not prompt evidence.
- Provider retry waits are bounded by `AH_THERE_IT_IS_LLM_MAX_RETRY_DELAY_SECONDS` (60 s default; 10 s in the Groq benchmark job). If an official `Retry-After` exceeds the configured cap, the OpenAI-compatible adapter fails immediately instead of sleeping through the whole Actions timeout. Safe `x-ratelimit-*` limit/remaining/reset headers are whitelisted into transport metadata; unrelated/auth headers are never persisted.
- Hard 429/quota failures are typed as `ProviderRateLimitError` for both Gemini and OpenAI-compatible providers. `live_eval` records the current failed case, writes the partial report, aborts the remaining batch, and lists unexecuted case IDs instead of burning more quota on known-unavailable capacity.
...
2. After Groq TPD capacity is available, run `tool-discipline` once with `inventory-v1` and once with `inventory-v2-strict` using the exact deterministic benchmark config/seed. Compare only reports with matching provider/model/config and inspect `system_fingerprint` before attributing differences to the prompt.
